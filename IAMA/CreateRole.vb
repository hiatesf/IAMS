...
Public Class CreateRole
    ...
    Public Sub CreateDept_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            strQuery = "SELECT Role FROM iamsrole"
    ...
    Private Sub DataGridView1_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellDoubleClick
        Try
            ...
            Dim strQuery As String
            strQuery = "SELECT * FROM iamsrole where Role = @Role"
            sqlCon = New SqlConnection(strConn)
            Using (sqlCon)
                Dim sqlComm As SqlCommand = New SqlCommand(strQuery, sqlCon)
                sqlComm.Parameters.AddWithValue("@Role", ID.ToString())
                sqlCon.Open()
    ...
    Private Sub Update_Click(sender As Object, e As EventArgs) Handles Update.Click
        Try
            ...
            Using (sqlCon)
                ...
                sqlComm.CommandText = "Updaterole"
                ...
                Dim StrQueryR As DataTable = refreshnow.RefreshmeGeneral("SELECT Role FROM iamsrole")
    ...
    Private Sub Save_Click(sender As Object, e As EventArgs) Handles Save.Click
        Try
            ...
            Using (sqlCon)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = sqlCon
                sqlComm.CommandText = "InsertIntorole"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("Role", Role.Text)
                sqlCon.Open()
                sqlComm.ExecuteNonQuery()
                sqlCon.Close()
                MessageBox.Show("A new role has been created successfully!", "Important Note", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Dim refreshnow As New RefreshmeGeneral()
                Dim StrQueryR As DataTable = refreshnow.RefreshmeGeneral("SELECT Role FROM iamsrole")
                DataGridView1.DataSource = StrQueryR
                For Each ctrl As Control In GroupBox1.Controls
